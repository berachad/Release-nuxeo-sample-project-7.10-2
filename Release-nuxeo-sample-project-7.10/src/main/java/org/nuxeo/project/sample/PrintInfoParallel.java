...
@Operation(id = PrintInfoParallel.ID, category = Constants.CAT_DOCUMENT, label = "Print Info")
public class PrintInfoParallel {
	...
	public static final String ID = "Document.PrintInfoParallel";
	private List<String> templatesNames = new ArrayList();

	@OperationMethod
	public void run() throws OperationException {
		System.out.println("parallel salma");
		...
		if (templateBasedDocument == null) {
			System.out.println("le document attestation ne contient pas les templates associ�");
		} else {
		...
		if (doc.getType().equals("FactureFournisseur")) {
			final String QUERY_PAYEES = "SELECT * FROM DocumentRoute WHERE docri:participatingDocuments IN ('"
					+ doc.getId() + "') and ecm:name like '%ParallelDocumentReview%' ";
			...
			System.out.println("doc.getType().equals(\"da\") : " + doc.getType().equals("da"));
...
//					+ "') and ecm:name like '%ParallelDocumentReview%' ";
//			System.out.println("QUERY_PAYEES : " + QUERY_PAYEES);
//
