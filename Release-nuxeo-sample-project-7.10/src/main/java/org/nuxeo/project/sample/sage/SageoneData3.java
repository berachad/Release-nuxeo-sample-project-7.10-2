 ...
 */
@WebServlet("/SageoneData3")
public class SageoneData3 extends HttpServlet {
	private static Log log = LogFactory.getLog(SageoneData3.class);
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			System.out.println("le endpoint >>>>>>>>>>>>>>>>><<<<"+endpoint+address_region_ide+"&visible_in=expenses");
			URIBuilder builder = new URIBuilder(endpoint+address_region_ide+"&visible_in=expenses");
...
//			resp.getWriter().println("</html>");
	...
	public static List<Object> toList(JSONArray array) throws JSONException {
	 ...
	 public Client createledger(LedgerAccounts ledger) throws KeyManagementException, NoSuchAlgorithmException, ClientProtocolException, IOException, JSONException
	 {			MethodShared = new MethodsShared();
				...
				WebResource webResource = client.resource(nouveauchaine + Constant.PATH_START_LEDGER + "/createLedger");
				Builder builder = webResource
						.accept(MediaType.APPLICATION_JSON)
						.header("content-type", MediaType.APPLICATION_JSON)
						.header("Authorization", "Bearer " + token);
				ClientResponse response = builder.put(ClientResponse.class, gson.toJson(ledger));
				log.error(" === < Request json object  create taxe : " + gson.toJson(ledger) + " > ===");
